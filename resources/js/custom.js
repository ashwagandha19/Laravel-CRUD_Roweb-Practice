...
$('#userCreateModal').on('shown.bs.modal', function(event) { 
    let button = $(event.relatedTarget);
    let modal = $(this);
})

$(document).ready(function() {
    $('#userCreateButton').on('click', function() {

        let name = $('#userCreateName').val();
        let email = $('#userCreateEmail').val();
        let password = $('#userCreatePassword').val();
        let role = $('#userCreateRole').val();
        ...
        $.ajax({
            ...
            data: {name: name,
                ...
                $("#userTableBody").prepend('<tr><td>'+ response.name +'</td><td>' + response.email + '</td><td>' + response.password + '</td><td>' + response.role + '</td>');
                $("#userForm")[0].reset();
            });
    });
})
    ...
    $('#boardCreateButton').on('click', function() {
        ...
        $.ajax({
            ...
            data: {name: name,
                ...
                $("#boardTableBody").prepend('<tr><td>'+ response.name +'</td></tr>');
                $("#boardForm")[0].reset();
            });
    });
})
    ...
    $('#taskCreateButton').on('click', function() {
        ...
        $.ajax({
            ...
            data: {name: name,
                ...
                $("#taskTableBody").prepend('<tr><td>'+ response.name +'</td></tr>');
                $("#taskForm")[0].reset();
            });
    });
})
